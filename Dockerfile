...
RUN curl -sl "https://archive.apache.org/dist/groovy/3.0.5/distribution/apache-groovy-binary-3.0.5.zip" --output /tmp/groovy-3.0.5.zip && unzip -q /tmp/groovy-3.0.5.zip -d /tmp && cp /tmp/groovy-3.0.5/lib/*.jar /opt/kafka/plugins/debezium-connector-mongodb/
