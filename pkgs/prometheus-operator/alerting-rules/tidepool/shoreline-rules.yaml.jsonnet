    ...
    rules: [
      {
        ...
        annotations: {
          message: 'High count({{ $value }}) of {{ $labels.status_code }} with the reason: {{ $labels.status_reason }} by the Shoreline pod: {{ $labels.namespace }}/{{ $labels.pod }}.',
      ...
      },
      {
        ...
        annotations: {
          message: 'High count({{ $value }}) of failed Marketo uploads by the Shoreline pod: {{ $labels.namespace }}/{{ $labels.pod }}.',
          dashboard: 'https://grafana.%s/d/5sv7jfiGk/shoreline?orgId=1&refresh=10s' % me.config.cluster.metadata.domain,
        },
