    ...
    rules: [
      {
        ...
        expr: 'sum(rate(envoy_cluster_upstream_rq_xx{envoy_response_code_class="5"}[2m])) by (envoy_cluster_name, namespace)  / sum(rate(envoy_cluster_upstream_rq_xx{envoy_response_code_class!="5"}[2m])) by (envoy_cluster_name, namespace)* 100 > 1',
        labels: {
          severity: 'critical',
        },
      },
